...
(function () {
  ...
  const {
    h, Card, SectionTitle, Pill, IconChip, Icon, Check,
    ProgressRing,
  } = window.GB;
  ...
  function ScoreCard({ score, tasks, habits, level }) {
    ...
    const ringNumber = h(
      'span',
      {
        style: {
          ...
          fontSize: '26px', color: 'var(--fg1)', lineHeight: 1,
        },
      },
      String(score)
    );
    ...
    const ring = ProgressRing({
      value: score, size: 92, color: 'var(--brand)',
      children: [ringNumber, ringLabel],
    });

    const heading = score >= 70
      ? "You're on track 🔥"
      : (score > 0 ? 'Keep it going' : 'Let’s get started');

    const pills = [];
    if (topStreak > 0) {
      pills.push(Pill({
        icon: 'flame', label: topStreak + '-day',
        bg: 'var(--coral-50)', fg: 'var(--coral-700)',
      }));
    }
    if (level && level > 0) {
      pills.push(Pill({
        icon: 'zap', label: 'Lvl ' + level,
        bg: 'var(--sun-50)', fg: 'var(--sun-700)',
      }));
    }
  ...
  function TasksCard({ tasks, toggleTask, onAdd }) {
      ...
      return Card({
        children: [
          h(
            'div',
            { class: 'gb-empty' },
            Icon('list-todo', { size: 26, color: 'var(--fg3)' }),
            h('p', null, 'No tasks yet.'),
            h(
              'button',
    ...
    return Card({ children: tasks.map(t => TaskRow(t, toggleTask)) });
  }
  ...
  function TodayPlanCard({ tasks, habits, water, wellness, onPlanToday }) {
    ...
    const waterPct = Math.min(100, Math.round((((water && water.consumedMl) || 0) / waterGoal) * 100));
    const chips = [
      pendingTasks + ' tasks',
      pendingHabits + ' habits',
      waterPct + '% water',
      sleep ? 'sleep set' : 'sleep open',
      mood ? 'mood set' : 'mood open',
    ];
    return Card({
      className: 'gb-coach-card',
      children: [
        h('div', { class: 'gb-coach-head' },
          h('div', null,
            h('div', { class: 'gb-coach-kicker' }, 'Daily brief'),
            h('div', { class: 'gb-coach-title' }, 'Help me plan today')
          ),
          h('button', { type: 'button', class: 'gb-btn gb-btn--primary gb-btn--compact', onclick: onPlanToday }, Icon('sparkles', { size: 16, sw: 2.5 }), 'Plan')
        ),
        h('div', { class: 'gb-coach-chiprow' }, chips.map(chip => h('span', { class: 'gb-coach-chip' }, chip)))
      ],
    });
  }
  ...
  function WellnessCard({ wellness, onAddSleep, onAddMood }) {
    ...
    return Card({
      ...
      children: [
        h('div', { class: 'gb-wellness-grid' },
          h('button', { type: 'button', class: 'gb-wellness-tile', onclick: onAddSleep },
          ...
          h('button', { type: 'button', class: 'gb-wellness-tile', onclick: onAddMood },
  ...
  function WeeklyReflectionCard({ tasks, habits, food, goals, gratitude, wellness }) {
    const doneTasks = (tasks || []).filter(t => t.done).length;
    ...
    const lines = [
      ...
      doneHabits + ' habits done today',
      actionCount + ' goal actions logged',
      (gratitude || []).length + ' gratitude notes',
      sleepDays + ' sleep logs and ' + moodDays + ' mood logs',
      kcal ? kcal + ' kcal logged today' : 'food log ready when needed',
    ];
    return Card({
      className: 'gb-reflect-card',
      children: [
        h('div', { class: 'gb-card-titleline' }, Icon('calendar-heart', { size: 18, sw: 2.4 }), 'Weekly reflection'),
        h('div', { class: 'gb-reflect-list' }, lines.map(line => h('div', { class: 'gb-reflect-row' }, line)))
      ],
    });
  }

  function BadgeCard({ tasks, habits, water, goals, gratitude, wellness }) {
    ...
    const badges = [
      ...
      { on: (gratitude || []).length > 0, icon: 'heart', label: 'Grateful' },
    ...
    return Card({
      ...
      children: [
        ...
        h('div', { class: 'gb-badge-grid' }, badges.map(badge => h('div', { class: 'gb-badge' + (badge.on ? ' is-on' : '') },
  ...
  function ReminderSuggestionsCard({ habits, water, wellness, onAddSuggestedReminder }) {
    ...
    if (missedHabit) items.push({ icon: 'repeat', text: missedHabit.name + ' reminder', time: '19:00', tag: 'health' });
    const waterGoal = (water && water.goalMl) || 2000;
    if (((water && water.consumedMl) || 0) < waterGoal * 0.5) items.push({ icon: 'droplets', text: 'Drink water', time: '16:00', tag: 'health' });
    if (!((wellness && wellness.sleepByDate) || {})[key]) items.push({ icon: 'moon', text: 'Sleep routine', time: '22:30', tag: 'personal' });
    if (!items.length) items.push({ icon: 'sparkles', text: 'Review tomorrow plan', time: '20:30', tag: 'personal' });
    return Card({
      className: 'gb-suggest-card',
      children: [
        h('div', { class: 'gb-card-titleline' }, Icon('bell-plus', { size: 18, sw: 2.4 }), 'Smart reminders'),
        h('div', { class: 'gb-suggest-list' }, items.slice(0, 3).map(item => h('div', { class: 'gb-suggest-row' },
          h('span', { class: 'gb-suggest-icon' }, Icon(item.icon, { size: 16, sw: 2.4 })),
          h('span', { class: 'gb-suggest-copy' }, h('strong', null, item.text), h('small', null, item.time)),
          h('button', { type: 'button', class: 'gb-btn gb-btn--soft gb-btn--compact', onclick: () => onAddSuggestedReminder && onAddSuggestedReminder(item.text, item.time, item.tag) }, 'Add')
        )))
      ],
    });
  }
  ...
  function GoalTimelineCard({ goals }) {
    const actions = allGoals(goals)
      .flatMap(goal => (goal.recentActions || []).map(action => ({ goal: goal.title, note: action.note, date: action.actionDate || String(action.createdAt || '').slice(0, 10) })))
      .sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')))
      .slice(0, 4);
    return Card({
      className: 'gb-timeline-card',
      children: [
        h('div', { class: 'gb-card-titleline' }, Icon('route', { size: 18, sw: 2.4 }), 'Goal timeline'),
        actions.length
          ? h('div', { class: 'gb-timeline-list' }, actions.map(action => h('div', { class: 'gb-timeline-row' },
              h('span', { class: 'gb-timeline-dot' }),
              h('span', { class: 'gb-timeline-copy' }, h('strong', null, action.note), h('small', null, action.goal + ' - ' + (action.date || 'Today')))
            )))
    ...
    function openNumericPrompt(opts) {
      ...
      const sheet = h(
        ...
        h(
          ...
          h('button', {
            type: 'button', class: 'gb-btn gb-btn--ghost', onclick: close,
          }, 'Cancel'),
          h('button', {
      ...
      children: [
        ...
        h(
          ...
          h(
            ...
            h('div', { class: 'gb-water-fill', style: { height: pct + '%' } },
          ...
          h(
            ...
            h('div', { class: 'gb-water-sub' }, remainingMl > 0 ? 'to hit your goal' : 'goal reached, amazing'),
        ...
        lastThree.length
          ? h(
              'div',
              { class: 'gb-water-log' },
              lastThree.map(item => h(
                'div',
                { class: 'gb-water-log-row' },
                h('div', { style: { flex: 1 } },
                  h('span', null, (item.amountMl || 0) + ' ml'),
                  h('span', { style: { marginLeft: '8px', fontSize: '12px', color: 'var(--fg3)' } }, timeLabel(item.loggedAt))
                ),
                h('button', {
                  type: 'button',
  ...
  function FoodCard({ food, onAddFood, onDeleteFood }) {
    ...
    return Card({
      ...
      children: [
        ...
        h(
          'button',
          { type: 'button', class: 'gb-water-add gb-food-add', onclick: () => onAddFood && onAddFood() },
          'Log food'
        ),
        recent.length
          ? h(
          ...
          : h('p', { class: 'gb-water-quote', style: { marginTop: '8px' } }, 'No food logged yet today.')
      ],
    });
  }
  ...
  function PhotoHistoryCard({ photoHistory }) {
    ...
    return Card({
      ...
      children: [
        h('div', { class: 'gb-water-head' },
          h('div', null,
            h('div', { class: 'gb-water-title' }, 'Photo food history'),
            h('div', { class: 'gb-water-meta' }, 'Recent plate estimates and confidence')
          ),
          h('div', { class: 'gb-food-total' }, items.length)
        ),
        items.length
          ? h('div', { class: 'gb-photo-history-list' }, items.slice(0, 6).map(item => h('div', { class: 'gb-photo-history-row' },
              h('span', { class: 'gb-photo-history-icon' }, Icon('camera', { size: 16, sw: 2.4 })),
              h('span', { class: 'gb-photo-history-copy' },
                h('strong', null, item.foodName || 'Plate photo'),
                h('small', null, (item.mealType || 'meal') + (item.confidence == null ? '' : ' - ' + item.confidence + '% confidence'))
              ),
              item.fallbackNeeded ? h('span', { class: 'gb-photo-history-pill' }, 'review') : h('span', { class: 'gb-photo-history-pill is-good' }, 'used')
            )))
          : h('p', { class: 'gb-water-quote', style: { marginTop: '8px' } }, 'Analyze a plate photo while logging food to build history.')
      ],
    });
  }
      ...
      return Card({
        children: [
          h(
            'div',
            { class: 'gb-empty' },
            Icon('repeat', { size: 26, color: 'var(--fg3)' }),
  ...
  function MiniCalendarCard({
    ...
    const dowRow = h('div', { class: 'gb-mini-cal-dow' },
    ...
    function daySection(title, count, items, emptyText) {
      return h(
        ...
        h('div', { class: 'gb-mini-cal-section-head' },
    ...
    if (dayFoodLoading && !selectedFood.length) {
      foodContent = h('div', { class: 'gb-mini-cal-empty' }, 'Loading food entries...');
    } else if (dayFoodError && !selectedFood.length) {
      foodContent = h('div', { class: 'gb-mini-cal-empty gb-mini-cal-empty--error' },
      ...
      foodContent = h('div', { class: 'gb-mini-cal-items' },
        selectedFood.map(item => h('div', { class: 'gb-mini-cal-item' },
          h('div', { class: 'gb-mini-cal-item-main' },
            h('div', { class: 'gb-mini-cal-item-title' }, item.foodName),
            h('div', { class: 'gb-mini-cal-item-sub' }, item.quantityGrams + 'g · ' + item.kcalEstimated + ' kcal')
          ),
    ...
    return Card({
      ...
      children: [
        h('div', { class: 'gb-mini-cal-header' },
          h('button', {
            type: 'button',
        ...
        h('div', { class: 'gb-mini-cal-legend' },
          h('span', { class: 'gb-mini-cal-legend-item' }, h('span', { class: 'dot has-tasks' }), 'Tasks'),
          h('span', { class: 'gb-mini-cal-legend-item' }, h('span', { class: 'dot has-reminders' }), 'Reminders'),
        ...
        daySection(
          ...
          selectedDueTasks.length
            ? h('div', { class: 'gb-mini-cal-items' }, selectedDueTasks.map(t => h('div', { class: 'gb-mini-cal-item' },
                h('div', { class: 'gb-mini-cal-item-main' },
        ...
        daySection(
          ...
          selectedCompletedTasks.length
            ? h('div', { class: 'gb-mini-cal-items' }, selectedCompletedTasks.map(t => h('div', { class: 'gb-mini-cal-item is-done' },
                h('div', { class: 'gb-mini-cal-item-main' },
                  h('div', { class: 'gb-mini-cal-item-title' }, t.title),
                  h('div', { class: 'gb-mini-cal-item-sub' }, 'Completed ' + prettyTaskTime(t.doneAt))
                )
              )))
            : null,
        ...
        foodSummary && typeof foodSummary.totalCalories === 'number'
          ? h('div', { class: 'gb-mini-cal-total' }, 'Total food: ' + foodSummary.totalCalories + ' kcal')
          : null,
        daySection(
          'Reminders',
          selectedReminders.length,
          selectedReminders.length
            ? h('div', { class: 'gb-mini-cal-items' }, selectedReminders.map(rem => h('div', { class: 'gb-mini-cal-item' },
                h('div', { class: 'gb-mini-cal-item-main' },
                  h('div', { class: 'gb-mini-cal-item-title' }, rem.text),
                  h('div', { class: 'gb-mini-cal-item-sub' },
                    formatTime(rem.time) + (rem.repeat && rem.repeat !== 'none' ? ' · ' + rem.repeat : '')
                  )
                ),
  ...
  function ScreenDashboard({ tasks, toggleTask, habits, toggleHabit, score, quote, water, food, goals, gratitude, wellness, reminders, foodSummary, dayFoodLoading, dayFoodError, onQuickAddWater, onUpdateWaterGoal, onAddFood, onDeleteWater, onDeleteFood, level, onAddTask, onAddHabit, timerWidget, calYear, calMonth, selectedDate, onSelectDate, onPrevMonth, onNextMonth, onRetryFood, onPlanToday, onAddSleep, onAddMood, onAddSuggestedReminder }) {
    return h(
      'div',
      { class: 'gb-rise gb-dash' },

      // ---- Main column: summary, tasks, focus timer ----
      h(
        'div',
        { class: 'gb-dash-col gb-dash-main' },

        h('div', { class: 'gb-dash-block' }, ScoreCard({ score, tasks, habits, level })),

        h('div', { class: 'gb-dash-block' }, TodayPlanCard({ tasks, habits, water, wellness, onPlanToday })),

        h('div', { class: 'gb-dash-block' }, WellnessCard({ wellness, onAddSleep, onAddMood })),
        ...
        timerWidget ? h('div', { class: 'gb-dash-block' }, timerWidget) : null
      ),

      // ---- Side column: habit streaks, quote ----
      h(
        'div',
        { class: 'gb-dash-col gb-dash-side' },
        ...
        h('div', { class: 'gb-dash-block' }, ReminderSuggestionsCard({ habits, water, wellness, onAddSuggestedReminder })),

        h('div', { class: 'gb-dash-block' }, WeeklyReflectionCard({ tasks, habits, food, goals, gratitude, wellness })),

        h('div', { class: 'gb-dash-block' }, BadgeCard({ tasks, habits, water, goals, gratitude, wellness })),

        h('div', { class: 'gb-dash-block' }, GoalTimelineCard({ goals })),

        h('div', { class: 'gb-dash-block' }, QuoteCard({ quote }))
      )
    );
  }

  function ScreenFood({ water, food, photoHistory, onQuickAddWater, onUpdateWaterGoal, onAddFood, onDeleteWater, onDeleteFood }) {
    return h(
      'div',
      ...
      h(
        'div',
        { class: 'gb-dash-block' },
        FoodCard({ food, onAddFood, onDeleteFood })
      ),
      h(
        'div',
        { class: 'gb-dash-block' },
        PhotoHistoryCard({ photoHistory })
      )
    );
  }

  window.GB.ScreenDashboard = ScreenDashboard;
  window.GB.RenderMiniCalendarCard = MiniCalendarCard;
  window.GB.ScreenFood = ScreenFood;
})();
