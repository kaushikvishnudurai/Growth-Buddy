...
(function () {
  ...
  const { h, Card, SectionTitle, Icon, Pill } = window.GB;
  ...
  const TAGS = {
    ...
    personal: { label: 'Personal', color: 'var(--iris-500)',  soft: 'var(--iris-50)',  softFg: 'var(--iris-700)' },
    health:   { label: 'Health',   color: 'var(--leaf-500)',  soft: 'var(--leaf-50)',  softFg: 'var(--leaf-700)' },
    urgent:   { label: 'Urgent',   color: 'var(--coral-500)', soft: 'var(--coral-50)', softFg: 'var(--coral-700)' },
    other:    { label: 'Other',    color: 'var(--sun-500)',   soft: 'var(--sun-50)',   softFg: 'var(--sun-700)' },
  };
  ...
  function occursOn(rem, key) {
    ...
    switch (rem.repeat) {
      ...
      case 'monthly': return t.d === a.d;
      case 'yearly':  return t.m === a.m && t.d === a.d;
      default:        return day.getTime() === anchor.getTime();
    }
  }
  ...
  function gratitudeOn(entries, key) {
    return (entries || []).filter(entry => entry.entryDate === key);
  }
      ...
      cells.push(
        h(
          'button',
          {
            ...
            class: cls,
            'data-day-key': key,
            'aria-label': prettyDate(key) + (todays.length ? ' — ' + todays.length + ' reminder(s)' : ''),
  ...
  function openDeleteDialog(rem, occKey, onDelete) {
    const opts = [
      ...
      { scope: 'future', icon: 'calendar-off', label: 'This & all future',   sub: 'From ' + prettyDate(occKey) + ' onward' },
      { scope: 'before', icon: 'history',      label: 'All before this day',  sub: 'Keep ' + prettyDate(occKey) + ' & later' },
    ...
    const sheet = h(
      'div',
      { class: 'gb-modal', role: 'dialog', 'aria-modal': 'true', 'aria-label': 'Delete recurring reminder' },
      h('div', { class: 'gb-modal-head' },
        h('div', { class: 'gb-modal-title' }, 'Delete recurring reminder'),
        h('div', { class: 'gb-modal-sub' }, '“' + rem.text + '” repeats ' + REPEATS[rem.repeat].label.toLowerCase() + '.')
      ),
      h('div', { class: 'gb-modal-opts' },
        opts.map(o =>
          h('button', {
            type: 'button',
            class: 'gb-modal-opt' + (o.scope === 'all' ? ' is-danger' : ''),
            onclick: () => { close(); onDelete(o.scope, rem.id, occKey); },
          },
            h('span', { class: 'gb-modal-opt-ic' }, Icon(o.icon, { size: 18 })),
            h('span', { class: 'gb-modal-opt-tx' },
  ...
  /* ---- Reminder row ---- */
  function ReminderRow(rem, occKey, onDelete) {
    const t = TAGS[rem.tag] || TAGS.other;
    const meta = [];
    if (rem.time) {
      meta.push(h('span', { class: 'meta-item' },
        Icon('clock', { size: 13, color: 'var(--fg3)' }), formatTime(rem.time)));
    }
    if (rem.repeat && rem.repeat !== 'none') {
      const reLabel = REPEATS[rem.repeat].label
        + (rem.until ? ' · until ' + prettyDate(rem.until) : '');
      meta.push(h('span', { class: 'meta-item' },
        Icon('repeat', { size: 13, color: 'var(--fg3)' }), reLabel));
    }
    ...
    return h(
      ...
      h(
        ...
        h(
          'div',
          { class: 'gb-rem-meta' },
          h('span', { class: 'gb-tag-pill', style: { background: t.soft, color: t.softFg } }, t.label),
          meta
        )
      ),
      h(
  ...
  function ReminderPanel({
    ...
    const dayGratitude = gratitudeOn(gratitude, selectedDate);
    ...
    const winCount = dayCompletedTasks.length + dayGoalActions.length + dayGratitude.length + dayFood.length + wellnessCount;
    ...
    if (list.length) {
      listNode = Card({
        children: list.map(r => ReminderRow(r, selectedDate, onDeleteReminder)),
      });
    } else {
      listNode = futureDate
        ? null
    ...
    if (dayTasks.length) {
      tasksNode = Card({
        children: dayTasks.map(t =>
          h('div', { class: 'gb-day-row' },
            h('div', { class: 'gb-day-row-main' },
              h('div', { class: 'gb-day-row-title' }, t.title),
              h('div', { class: 'gb-day-row-sub' }, prettyTaskTime(t.dueAt) + (t.done ? ' · done' : ''))
            ),
      ...
      tasksNode = futureDate ? null : h('div', { class: 'gb-day-empty' }, 'No due tasks for this day.');
    }
    ...
    if (dayFoodLoading && !dayFood.length) {
      ...
      foodNode = h('div', { class: 'gb-day-empty gb-day-empty--error' },
        ...
        children: dayFood.map(f =>
          h('div', { class: 'gb-day-row' },
            h('div', { class: 'gb-day-row-main' },
              ...
              h('div', { class: 'gb-day-row-sub' }, f.quantityGrams + 'g · ' + f.kcalEstimated + ' kcal')
            ),
    ...
    const winItems = [];
    dayCompletedTasks.slice(0, 3).forEach(t => winItems.push({ icon: 'check-circle-2', title: t.title, sub: 'Task completed' }));
    dayGoalActions.slice(0, 3).forEach(a => winItems.push({ icon: 'target', title: a.note, sub: 'Progress on ' + a.goal }));
    dayGratitude.slice(0, 3).forEach(g => winItems.push({ icon: 'heart', title: g.note, sub: 'Gratitude note' }));
    if (daySleep) winItems.push({ icon: 'moon', title: (daySleep.bedtime || '--') + ' to ' + (daySleep.wakeTime || '--'), sub: 'Sleep logged' });
    if (dayMood) winItems.push({ icon: 'smile-plus', title: dayMood.mood + ' mood, ' + dayMood.energy + ' energy', sub: 'Mood check-in' });
    dayFood.slice(0, 2).forEach(f => winItems.push({ icon: 'utensils', title: f.foodName, sub: (f.kcalEstimated || 0) + ' kcal logged' }));
    const winsNode = winCount
      ? Card({
          className: 'gb-wins-card',
          children: [
            h('div', { class: 'gb-wins-score' },
              h('span', { class: 'gb-wins-number' }, String(winCount)),
              h('span', { class: 'gb-wins-copy' }, winCount === 1 ? 'win saved for this day' : 'wins saved for this day')
            ),
            h('div', { class: 'gb-wins-list' }, winItems.slice(0, 6).map(item => h(
              'div',
              { class: 'gb-wins-row' },
              h('span', { class: 'gb-wins-icon' }, Icon(item.icon, { size: 15, sw: 2.4 })),
              h('span', { class: 'gb-wins-text' },
      ...
      : h('div', { class: 'gb-wins-empty' },
          Icon('sparkles', { size: 24, color: 'var(--fg3)' }),
          h('p', null, 'No wins saved here yet. Finish a task, log food, add a goal action, or write gratitude.')
        );

    const totalCount = list.length + dayTasks.length + (!futureDate ? dayFood.length : 0);
    const futurePlanningHint = futureDate && !dayTasks.length && !list.length
      ? h('div', { class: 'gb-future-note' },
    ...
    return h(
      ...
      futurePlanningHint,
      !futureDate ? h('div', { class: 'gb-cal-block gb-day-section', 'data-cal-section': 'wins' },
          h('div', { class: 'gb-day-head' },
            h('span', { class: 'gb-day-head-title' }, 'Past wins'),
            h('span', { class: 'gb-day-head-count' }, String(winCount))
          ),
          winsNode
        ) : null,
      h('div', { class: 'gb-cal-block gb-day-section', 'data-cal-section': 'tasks' },
        h('div', { class: 'gb-day-head' },
      ...
      !futureDate ? h('div', { class: 'gb-cal-block gb-day-section', 'data-cal-section': 'food' },
        h('div', { class: 'gb-day-head' },
      ...
      h('div', { class: 'gb-cal-block gb-day-section', 'data-cal-section': 'reminders' },
        h('div', { class: 'gb-day-head' },
  ...
  function ScreenCalendar({
    year, month, selectedDate, reminders, tasks, goals, gratitude, wellness, foodSummary, dayFoodLoading, dayFoodError,
    onPrevMonth, onNextMonth, onToday,
    onSelectDate, onRetryFood, onAddReminder, onDeleteReminder,
  }) {
    const toolbar = CalendarToolbar({ year, month, reminders, tasks, onPrevMonth, onNextMonth, onToday });
  ...
  function CalendarToolbar({ year, month, reminders, tasks, onPrevMonth, onNextMonth, onToday }) {
    ...
    return h(
      ...
      h(
        ...
        h(
          'button',
          { type: 'button', class: 'gb-iconbtn', 'aria-label': 'Previous month', onclick: onPrevMonth },
      ...
      h(
        ...
        monthCount
          ? Pill({ icon: 'bell', label: monthCount + ' items this month', bg: 'var(--brand-soft)', fg: 'var(--brand-soft-fg)' })
          : null,
        h(
          'button',
  ...
  window.GB.ScreenCalendar = ScreenCalendar;
  window.GB.RenderCalendarToolbar = CalendarToolbar;
  window.GB.RenderCalendarSide = ReminderPanel;
  window.GB.RenderCalendarGrid = MonthGrid;
  window.GB.resetCalendarForm = resetCalendarForm;
})();
