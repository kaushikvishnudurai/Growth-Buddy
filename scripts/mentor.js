...
(function () {
  ...
  function typingBubble() {
    return h(
      ...
      h('div', { class: 'gb-msg-avatar' }, Icon('sparkles', { size: 16, color: '#fff' })),
      h('div', { class: 'gb-msg-bubble is-bot gb-msg-typing' },
        h('span', { class: 'd' }), h('span', { class: 'd' }), h('span', { class: 'd' }))
    );
  }
  ...
  function ScreenMentor({ api }) {
    ...
    function renderMessages(msgs) {
      ...
      if (!msgs.length) {
        listEl.appendChild(h('div', { class: 'gb-mentor-empty' },
          ...
          h('div', { class: 'gb-mentor-suggestions' },
            ['I feel overwhelmed', 'Help me plan today', 'I keep procrastinating'].map(p =>
              h('button', {
                type: 'button',
                class: 'gb-btn gb-btn--soft gb-mentor-suggestion',
                onclick: () => { input.value = p; send(); },
              }, p)
            )
          )
        ));
    ...
    async function clearAll() {
      ...
      const ok = window.confirm('Clear all mentor messages in this chat?');
      if (!ok) return;
      busy = true;
      try {
        await api.clear();
    ...
    const chipsRow = h(
      ...
      PROMPTS.map(p =>
        h('button', {
          type: 'button',
          class: 'gb-msg-chip',
          onclick: () => { input.value = p; input.focus(); send(); },
        }, p)
      )
    );
  ...
  window.GB.ScreenMentor = ScreenMentor;
})();
