...
- **There is exactly one Settings modal.** `openProfileSettings` owns all five tabs — Profile,
  Display, Alerts, Layout, Account. Alerts holds push, the notification-sound picker (`segmented` over `CHIMES` from `chime.js`, saved to `ui_prefs.notifySound` and previewed on tap), WhatsApp, the digest and the working week.
